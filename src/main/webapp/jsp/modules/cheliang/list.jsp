            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">车辆列表</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <div class="table-responsive mb-3">
                            ...
                            <table id="tableId" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th onclick="sort('name')">车辆牌照</th>
                                    <th onclick="sort('brand')">品牌</th>
                                    <th onclick="sort('design')">款式</th>
                                    <th>车辆图片</th>
                                    <th onclick="sort('money')">租赁价格/时</th>
                                    <th onclick="sort('age')">车龄</th>
                                    <th onclick="sort('ztTypes')">车辆状态</th>
                                    <th onclick="sort('miaoshuContent')">车辆描述</th>
                                    <th>操作</th>
                                </tr>
                                </thead>
                                <tbody>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">电车租赁</h5>
...
<script>
    ...
    function setDataRow(item, number) {
        ...
        var designCell = document.createElement('td');
        designCell.innerHTML = item.design;
        ...
        var miaoshuContentCell = document.createElement('td');

        //如果图文详情包含图片
        if (item.miaoshuContent && item.miaoshuContent.indexOf('img') != -1) {
            //暂时只考虑图片+文字 和 文字+图片的情况
            var beginIndex = item.miaoshuContent.indexOf('<img');
            var endIndex = item.miaoshuContent.indexOf('>');
                miaoshuContentCell.innerHTML = item.miaoshuContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
        } else {
            if (item.miaoshuContent != null && item.miaoshuContent != "" && item.miaoshuContent.length >= 11) {
                    miaoshuContentCell.innerHTML = item.miaoshuContent.substring(0, 10) + "...";
            } else {
                    miaoshuContentCell.innerHTML = item.miaoshuContent;
            }
        }

        row.appendChild(miaoshuContentCell);
        ...
        if(item.ztTypes == 2){
            ...
            detailBtn.setAttribute("class", "btn btn-info btn-sm 租赁");
            detailBtn.innerHTML = "租赁"
    ...
    // 租赁
    function lease(id,day) {
        var mymessage = confirm("真的要租赁吗？");
        if (mymessage == true) {
            http("cheliang/lease?ids="+id+"&day="+day, "get", {}, (res) => {
                if(res.code == 0
        )
            {
                getDataList();
    ...
    function crossTable(id, crossTableName) {
        ...
        window.sessionStorage.setItem('crossTableName', "cheliang");
